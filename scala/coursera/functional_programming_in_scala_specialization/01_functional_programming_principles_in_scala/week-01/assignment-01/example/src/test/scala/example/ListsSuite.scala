...
class ListsSuite {
  ...
  @Test def `sum of an empty List must be zero`: Unit = {
    Assert.assertEquals(0, sum(List()))
  }

  @Test def `sum of List with one element`: Unit = {
    Assert.assertEquals(1, sum(List(1)))
  }

  @Test def `sum of doubled elements in the List`: Unit = {
    Assert.assertEquals(4, sum(List(2, 2)))
  }
